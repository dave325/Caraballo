...
<?php 
	...
	if (isset ( $plugin_options ['ddata_gallery_selection_rows'] )) {
		...
		$output .= "<div class='gallery' id='ddata-portfolio-container'>";
	} else {
	...
	foreach ( $link_image as $k => $v ) {
		...
		if (isset($v['image']) && strlen($v['image']) > 2){
			$output .= "<img src='{$v['image']}' alt='{$k}' id='{$key}'height='{$v['height']}px' width='{$v['width']}%' />";
		}
		if (isset ( $v ['link'] ) && strlen ( $v ['link'] ) > 4) {
			$output .= "</a>";
		}
